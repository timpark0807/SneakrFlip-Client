...
function PropertyForm() { 
    ...
    const [fields, setFields] = React.useState({address: '',
                                                zipcode: '',
                                                price: '',
                                                category:''
                                            })
    ...
    const handleSubmit = event => {
        Axios.post("http://localhost:8000/api/property",  fields, {headers: {"Authorization":"Bearer " + Cookies.get("token")}})
        .then(response => {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <div>
                <div>
                <TextField id="standard-basic" label="Address" name="address" value={fields.address} onChange={changeHandler}></TextField>
                </div>

                <div>
                <TextField id="standard-basic" label="Zipcode" name="zipcode" value={fields.zipcode} onChange={changeHandler}></TextField>
                </div>

                <div>
                <TextField id="standard-basic" label="Purchase Price" name="price" value={fields.price} onChange={changeHandler}></TextField>
                </div>
                <div>
